...
### Key points
- [implementation](https://github.com/yegorskikh/IOS-Data-Structures-and-Algorithms/blob/main/Section%201.%20Introduction/Complexity/Complexity.swift)

### Что это такое

Сложность показывает, как меняется время выполнения и потребление памяти алгоритма при росте входных данных.
...
- Big O применяется и к времени, и к памяти  
- Big O - это не про секунды, а про рост  
- на маленьких данных разница может быть незаметна  
- константы тоже важны (иногда O(n) быстрее O(log n))  

---

### 💡 Практика

- один цикл по данным - чаще всего O(n)  
- вложенные циклы - чаще всего O(n²)  
- деление задачи пополам - обычно O(log n)  

---

### 💡 Как оценивать память

- если создаёшь новую структуру данных - обычно O(n)  
- если работаешь "на месте" (in-place) - O(1)  
- рекурсия использует стек вызовов - от O(log n) до O(n)  

---

### 💡 Примеры по памяти (на коде)
...
#### O(log n) - память из-за рекурсии

```swift
func binarySearch(_ array: [Int], target: Int) -> Int? {
    return binarySearch(array, target: target, left: 0, right: array.count - 1)
}
...
private func binarySearch(
    ...
    if array[mid] == target {
        ...
        return binarySearch(array, target: target, left: left, right: mid - 1)
    } else {
        return binarySearch(array, target: target, left: mid + 1, right: right)
    }
}
```
...
- поиск элемента в `Array` через `first(where:)` - O(n)  
- проверка наличия элемента в `Set` - O(1)  
- доступ к значению в `Dictionary` по ключу - O(1)  

---

### 💡 Баланс времени и памяти

- Merge Sort - O(n log n) по времени, но требует O(n) памяти  
- Quick Sort - O(n log n) в среднем, использует O(log n) памяти  

💡 В реальной разработке почти всегда ищется баланс между скоростью и памятью  

---

💡 Вывод:
> если операция выполняется часто - её сложность критична

---

💡 Главное правило:
> сначала оцени сложность, потом пиши код
...
- вставка/удаление в начале - O(n)  
- поиск элемента (`contains`) - O(n)  

💡 хранит элементы подряд в памяти, поэтому хорошо работает с кэшем
...
💡 основан на хешировании (Hashable)
...
- нужен порядок элементов - используй `Array`  
- нужен быстрый поиск уникальных значений - используй `Set`  
- нужен доступ по ключу - используй `Dictionary`  

---

### 💡 Примеры из iOS

- список элементов для отображения (UITableView / SwiftUI List) - `Array`  
- проверка, был ли уже добавлен элемент (например, id) - `Set`  
- кэш объектов по id (например, модели с сервера) - `Dictionary`  

---

### 💡 Частая ошибка

> использовать `Array` там, где нужен `Set` или `Dictionary`
...
- `Array` - удобен, но медленный для поиска  
- `Set` - лучший выбор для проверки уникальности  
- `Dictionary` - лучший выбор для доступа по ключу  

💡 правило:
> если тебе нужен быстрый поиск - почти всегда это не Array
...
Стек - это структура данных типа **LIFO** (последний пришёл - первый вышел).
...
- undo / redo  
- парсинг выражений  
- рекурсия (через стек вызовов)  

---

💡 Вывод:
> стек нужен там, где важен порядок “последний добавленный - первый обработанный”

---

## **LinkedList**

![list](https://user-images.githubusercontent.com/60622982/128637581-ad7be8b0-8204-4064-96be-b14d4b36580b.png)
...
Связанный список - это структура, где каждый элемент хранит ссылку на следующий.
...
- хуже работает с кэшем по сравнению с Array  
- нельзя быстро перейти к произвольному элементу  

---

### 💡 Когда использовать

- когда часто вставляешь/удаляешь элементы в начале  
- когда не нужен быстрый доступ по индексу  

---

💡 Реальность:
> в iOS чаще используется Array, но LinkedList важен для понимания структуры данных

---

## **Queues**

![queue](https://user-images.githubusercontent.com/60622982/128637661-8a24d542-5dd7-4990-974a-de456b6bfc09.png)
...
- enqueue - O(1)  
- dequeue - O(1) (при правильной реализации)  
...
💡 Вывод:
> очередь нужна там, где важен порядок “первый пришёл - первый обработан”
